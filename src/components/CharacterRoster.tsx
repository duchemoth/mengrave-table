...
export function CharacterRoster({
    ...
    const spentSkillPoints = selectedCharacter
        ? Object.values(selectedCharacter.skills).reduce((sum, value) => sum + value, 0)
        : 0;
                                ...
                                <section className="character-editor-section">
                                    <div className="character-skills-header">
                                        <div>
                                            ...
                                            <h3 className="character-section-title">Навыки персонажа</h3>
                                        </div>
                                    ...
                                    <p className="character-help-text">
                                        Стартовая рекомендация: 12 очков навыков, максимум 3 в одном навыке.
                                        Превышение не блокируется, но подсвечивается для контроля мастера.
                                    </p>
                                    ...
                                    {(hasTooManySkillPoints || hasSkillAboveStartingMax) && (
                                        <div className="character-warning-box">
                                            {hasTooManySkillPoints && (
                                                <p>
                                                    Потрачено больше 12 очков. Это нормально для опытного персонажа,
                                                    но не для стандартного старта.
                                                </p>
                                            )}

                                            {hasSkillAboveStartingMax && (
                                                <p>
                                                    Один или несколько навыков выше 3. Для стартового персонажа это
                                                    обычно недоступно без особого разрешения.
                                                </p>
                                            )}
                                        </div>
                                    )}
                                    ...
                                    <div className="character-skills-grid">
                                        {CHARACTER_SKILL_LABELS.map((skill) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    <span className="character-skill-name">{skill.label}</span>

                                                    <div className="character-skill-controls">
                                                        <button
                                                            type="button"
                                                            onClick={() => updateSelectedSkill(skill.key, value - 1)}
                                                        >
                                                            −
                                                        </button>
